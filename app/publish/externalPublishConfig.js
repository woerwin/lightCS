...
function runnable(){
    ...
    config['pubnews'] = {
        fetchUrl : function(page){//page 订阅时传的参数
            return 'http://www.cnblogs.com/lhb25/default.html?page='+page;
            //return 'http://news.baidu.com/';
        },
        
        resolve : function(originData){
            decodeData = iconv.decode(originData,'utf-8');
            var topnewsRegex = /class="postTitle2" href="([\W\w]*?)">([\W\w]*?)<\/a>/g;
            var oneRegex = /class="postTitle2" href="([\W\w]*?)">([\W\w]*?)<\/a>/;
            var topnews = decodeData.match(topnewsRegex);
            var str = '';
            for(var i = 0; i < topnews.length; i++){
                var matchInfo = topnews[i].match(oneRegex);
                str += '<li><a href="'+ matchInfo[1] + '" target="_blank">' + matchInfo[2] + '</a></li>';
            }
            var resolved = {
                topnews: str
            }
